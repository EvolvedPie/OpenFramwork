...
void ofApp::setup(){
    ...
    for (int y = 0; y<height-1; y++){
        for (int x=0; x<width-1; x++){
            mainMesh.addIndex(x+y*10);                // 0
            mainMesh.addIndex((x+1)+y*10);            // 1
            mainMesh.addIndex(x+(y+1)*10);            // 10
            
            mainMesh.addIndex((x+1)+y*10);            // 1
            mainMesh.addIndex((x+1)+(y+1)*10);        // 11
            mainMesh.addIndex(x+(y+1)*10);            // 10
        }
    }
...
void ofApp::update(){
    ...
    if (bmessyMesh) {
        for (int i = 0; i < mainMesh.getNumVertices(); i++){ // find the total of all the vertices in the mesh and loop through them
            ...
            newPosition.z = ofRandom(-100.0, 1.0); // set the z value of it to a new random number
            mainMesh.setVertex(i, newPosition); // update the position of the vertex with the new co-ordinates
            
        }/*
		for (int i = 0; i < mainMesh.getNumVertices(); i++){ // find the total of all the vertices in the mesh and loop through them
            ofVec3f newPosition = mainMesh.getVertex(i); // get the current x,y,z position of this vertex
            newPosition.x = ofRandom(-100.0, 1.0); // set the z value of it to a new random number
            mainMesh.setVertex(i, newPosition); // update the position of the vertex with the new co-ordinates
            
        }
		for (int i = 0; i < mainMesh.getNumVertices(); i++){ // find the total of all the vertices in the mesh and loop through them
            ofVec3f newPosition = mainMesh.getVertex(i); // get the current x,y,z position of this vertex
            newPosition.y = ofRandom(-100.0, 1.0); // set the z value of it to a new random number
            mainMesh.setVertex(i, newPosition); // update the position of the vertex with the new co-ordinates
